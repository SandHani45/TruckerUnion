    ...
    <div class="container">
      <div class="row pd-top-15">
        <div class="col-sm-12 mt-0">
          ...
          <h4 class="text-capitalize" ><b> {{ Auth::user()->name }} </b> Group Member's</h4>
    ...
    <div class="row">
          <div class="col-md-9 ml-5 over-flow">
            ...
            @foreach($userDatas as $userData)
              <tr>
                <td class="media" style="width: 75%">
                  <i class="fa fa-comments-o group-icon" aria-hidden="true"></i>
                  <a class="drop" href="">{{ $userData->phone_number }} </a>
                </td>
                <td style="width: 25%"><a  class="red-icon" href="" onclick="
                                  if(confirm('Are sure, you want to delete this?'))
                                  {
                                    event.preventDefault();
                                    document.getElementById('delete-form-{{$userData->id}}').submit();
                                  }else
                                  {
                                    event.preventDefault();
                                  }"> &#9940; {{ 'Remove'}}
                    ...
                    <form id="delete-form-{{$userData->id}}" action="{{ route('group-member.destroy', $userData->id) }}" style="display: none;" method="post">
                      @csrf
                      {{method_field('DELETE')}}
                    </form>
                </td>
            ...
            <tr >
                <td class="media" style="width: 75%">
                  <i class="fa fa-comments-o group-icon" aria-hidden="true"></i>
                  <a class="drop" href="">No Result </a></td>
              </tr>
            @endif
             
            </table>
          </div>
             <div class="col-md-2 search-set" style="margin-top: 255px;">
         <div class="add-drop">  
            <a href="{{ route('drop_points.create')}}" class="pd-drop" >
            <img src="" alt=""> </a>
          </div>
       </div> 
    </div>
